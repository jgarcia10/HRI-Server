...
IP="${1:-192.168.131.140}"
ok(){ printf '  \033[32m✔\033[0m %s\n' "$*"; }; bad(){ printf '  \033[31m✘\033[0m %s\n' "$*"; }
echo "UR5 preflight → $IP"
if ip -br addr | grep -E "^(en|eth)" | grep -q "192.168.131\."; then ok "wired interface has a 192.168.131.x address: $(ip -br addr | grep -E '^(en|eth)' | awk '{print $3}')"
else bad "no wired 192.168.131.x address — plug the cable and run:  sudo nmcli con up ursky   (or set a static IP 192.168.131.10/24)"; fi
...
try:
    for c in ("PolyscopeVersion", "robotmode", "safetystatus", "is in remote control", "programState"):
        print(f"  · {c}: {dash(c)}")
...
try:
    ...
    print("  ✔ RTDE control: connected (Remote Control mode OK)"); c.disconnect()
except Exception as e:
